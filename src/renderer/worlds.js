...
(async function () {
  ...
  const colorsFetch = await fetch(colorsUrl);
  ...
  class Block {
    ...
    loadTexture(callback) {
      if (this.texture === "Plain") {
        ...
        loader.load(
          "https://bwsecondary.ddns.net/uploads/Texture2D/" +
            this.texture +
            ".png",
          (texture) => {
            callback(texture);
    ...
    loadObj(callback) {
      if (this.type === "Cube") {
        ...
        objLoader.load(
          "https://bwsecondary.ddns.net/uploads/Blocks/" + this.type + ".obj",
          (root) => {
            callback(root);
          },
          (xhr) => {},
        ...
        this.loadObj((root) => {
          this.loadTexture((texture) => {
            ...
            for (let i = this.paints.length; i > 0; i--) {
              ...
              let color =
                colors[paint] === undefined
                  ? 0xffffff
                  : rgbToHex(colors[paint]);
  ...
  window["loadWorld"] = function (localId) {
    const meta = JSON.parse(
      fs.readFileSync(worldsPath + localId + "/metadata.json"),
    );
    const encrypted = fs
      .readFileSync(worldsPath + localId + "/source.bw")
      .slice(2);
  ...
  console.log("Reading worlds path: " + worldsPath);
  fs.readdir(worldsPath, (err, files) => {
    ...
    files.forEach((file) => {
      const world = JSON.parse(
        fs.readFileSync(worldsPath + file + "/metadata.json"),
      );
